...
const TaskModal = ({
  ...
  const handleEdit = () => {
    // Set up the editing task properly
    if (task && !editingTask) {
      // We're editing an existing task from view mode
      setIsEditing(true);
    }
  };

  // Determine what task data to use and whether we're in editing mode
  const isInEditMode = isEditing || !!editingTask;
  ...
  const handleCancel = () => {
    ...
    console.log("editingTask?.isNew:", editingTask?.isNew);
    console.log("isEditing:", isEditing);

    if (editingTask?.isNew) {
      // If it's a new task, close the entire modal
      console.log("Closing modal for new task");
      onClose();
    } else {
      // If editing existing task, go back to view mode
      console.log("Going back to view mode");
      setIsEditing(false);
    }
  };
...
const TaskDetails = ({ task, onEdit, onClose, allTasks = {} }) => {
  const getTaskTitle = (taskId) => {
    const taskObj = allTasks[taskId];
    return taskObj ? `${taskObj.ticketId} - ${taskObj.title}` : taskId;
  };

  const getShortTaskTitle = (taskId, maxLength = 35) => {
    const fullTitle = getTaskTitle(taskId);
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center space-x-3 mb-2">
            <span
              className={`px-3 py-1 text-sm font-medium rounded border bg-${
                task.type === "BUG"
                  ? "red"
                  : task.type === "FEATURE"
                  ? "green"
                ...
                <div className="space-y-1">
                  {task.dependsOn.map((dep, index) => (
                    <div
                      ...
                      title={getTaskTitle(dep)}
                    >
                      {getShortTaskTitle(dep)}
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-1">
                  {task.blocks.map((block, index) => (
                    <div
                      key={index}
                      className="text-sm text-gray-600 bg-gray-50 px-2 py-1 rounded"
                      title={getTaskTitle(block)}
                    >
                      {getShortTaskTitle(block)}
                    </div>
                  ))}
                </div>
              </div>
